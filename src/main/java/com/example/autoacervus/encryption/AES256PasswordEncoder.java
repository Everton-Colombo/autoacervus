...
public class AES256PasswordEncoder implements PasswordEncoder {
    ...
    // It may seem like this class could be implemented with only static methods.
    // However, it can't, as to match spring's PasswordEncoder interface method
    // signatures. Therefore, it is made to be a singleton.
    // (This also satisfies one of the assignment's requirements, so that's pretty
    // cool)
    ...
    public static AES256PasswordEncoder getInstance() {
        return instance;
    }
    ...
    public boolean matches(CharSequence rawPassword, String encodedPassword) {
        ...
        // Although it would be ideal to encode the received rawPassword instead of
        // decoding the already encoded password, the AES256 algorithm used encodes
        // implements internal salting, meaning that encrypting the same password with
        // the same salt (our salt, external to the AES256 implementation) produces
        // different outputs.
    ...
    // Usually, PasswordEncoders shouldn't have a decode function, as one-way
    // hashing should be used most of the time. In the scope of this project,
    // however, it does make sense, as the decoded passwords are needed to
    // communicate with the acervus api.
